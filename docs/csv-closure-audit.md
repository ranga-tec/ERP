...
| 3.3 Reorder Planning | Partial | reorder settings + reorder alerts page/API + dashboard KPI | Suggest quantity and direct PR/PO generation workflow not implemented |
...
| 7) Reporting & Analytics | Partial | `ReportingController` dashboard KPI endpoint + dashboard page; AR/AP and inventory views exist | Advanced reports listed in CSV (stock ledger, valuation, supplier performance, profitability, aging, tax reports, service KPIs) are mostly not implemented yet |
...
| Notes: "All documents support status/PDF/attachments/comments/activity logs" | Partial | Broad PDF support, audit logs, document collaboration panel rolled out to many detail pages | "All documents" claim needs strict page-by-page verification; attachment upload safety hardening is still in progress |
