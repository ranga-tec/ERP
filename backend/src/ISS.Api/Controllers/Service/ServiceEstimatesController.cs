    ...
    IDocumentPdfService pdfService) : ControllerBase
{
    public sealed record ServiceEstimateSummaryDto(
        ...
        ServiceEstimateCustomerApprovalStatus CustomerApprovalStatus,
        DateTimeOffset? SentToCustomerAt,
        DateTimeOffset? CustomerDecisionAt,
        decimal Subtotal,
        decimal TaxTotal,
        decimal Total,
        int LineCount);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var rows = await dbContext.ServiceEstimates.AsNoTracking()
            ...
            .Select(x => new ServiceEstimateSummaryDto(
                ...
                x.RevisionNumber,
                x.Status,
                x.CustomerApprovalStatus,
                x.SentToCustomerAt,
                x.CustomerDecisionAt,
    ...
    public async Task<ActionResult<ServiceEstimateDto>> Get(Guid id, CancellationToken cancellationToken)
    {
        ...
        return Ok(new ServiceEstimateDto(
            ...
            estimate.Status,
            estimate.CustomerApprovalStatus,
            estimate.SentToCustomerAt,
