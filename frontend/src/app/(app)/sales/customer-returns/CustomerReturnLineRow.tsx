...
export function CustomerReturnLineRow({
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await apiPutNoContent(`sales/customer-returns/${customerReturnId}/lines/${line.id}`, {
        quantity: qty,
        unitPrice: price,
  ...
  async function deleteLine() {
    if (!window.confirm("Delete this customer return line?")) return;

    setError(null);
    setBusy(true);
    try {
      await apiDeleteNoContent(`sales/customer-returns/${customerReturnId}/lines/${line.id}`);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      setBusy(false);
