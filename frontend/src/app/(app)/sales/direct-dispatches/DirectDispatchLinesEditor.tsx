...
type EditableDirectDispatchLine = {
  id: string;
  itemId: string;
  quantity: string;
  batchNumber: string;
...
type DirectDispatchLinesEditorProps = {
...
function toEditableLines(lines: DirectDispatchLineDto[]): EditableDirectDispatchLine[] {
...
export function DirectDispatchLinesEditor({
  directDispatchId,
  warehouseId,
  warehouses,
  lines,
  ...
  async function saveLine(lineId: string) {
    ...
    try {
      ...
      await apiPutNoContent(`sales/direct-dispatches/${directDispatchId}/lines/${lineId}`, {
        quantity,
  ...
  async function deleteLine(lineId: string) {
    if (!window.confirm("Delete this direct dispatch line?")) {
      return;
    }
    ...
    try {
      await apiDeleteNoContent(`sales/direct-dispatches/${directDispatchId}/lines/${lineId}`);
  ...
  const baseColumns = useMemo<EditableDataTableColumn<EditableDirectDispatchLine>[]>(() => [
    {
      key: "item",
      header: "Item",
      kind: "display",
