...
export function DirectDispatchLineRow({
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await apiPutNoContent(`sales/direct-dispatches/${directDispatchId}/lines/${line.id}`, {
        quantity: qty,
  ...
  async function deleteLine() {
    if (!window.confirm("Delete this direct dispatch line?")) return;

    setError(null);
    setBusy(true);
    try {
      await apiDeleteNoContent(`sales/direct-dispatches/${directDispatchId}/lines/${line.id}`);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      setBusy(false);
