...
function toEditableLines(lines: QuoteLineDto[]): EditableQuoteLine[] {
...
export function QuoteLinesEditor({
  ...
  const [draftLines, setDraftLines] = useState<EditableQuoteLine[]>(() => toEditableLines(lines));
  ...
  function updateRow(lineId: string, updater: (line: EditableQuoteLine) => EditableQuoteLine) {
  ...
  async function saveLine(lineId: string) {
    ...
    try {
      ...
      await apiPutNoContent(`sales/quotes/${quoteId}/lines/${lineId}`, {
        quantity,
        unitPrice,
      });
  ...
  async function deleteLine(lineId: string) {
    if (!window.confirm("Delete this quote line?")) {
      return;
    }
    ...
    try {
      await apiDeleteNoContent(`sales/quotes/${quoteId}/lines/${lineId}`);
  ...
  const baseColumns = useMemo<EditableDataTableColumn<EditableQuoteLine>[]>(() => [
    {
      key: "item",
      header: "Item",
      kind: "display",
