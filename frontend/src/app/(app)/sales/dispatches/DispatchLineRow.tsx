...
export function DispatchLineRow({
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await apiPutNoContent(`sales/dispatches/${dispatchId}/lines/${line.id}`, {
        quantity: qty,
  ...
  async function deleteLine() {
    if (!window.confirm("Delete this dispatch line?")) return;

    setError(null);
    setBusy(true);
    try {
      await apiDeleteNoContent(`sales/dispatches/${dispatchId}/lines/${line.id}`);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      setBusy(false);
