...
export function SupplierReturnLineRow({
  supplierReturnId,
  line,
  itemLabel,
  canEdit,
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await apiPutNoContent(`procurement/supplier-returns/${supplierReturnId}/lines/${line.id}`, {
        quantity: qty,
        unitCost: cost,
  ...
  async function deleteLine() {
    if (!window.confirm("Delete this supplier return line?")) return;

    setError(null);
    setBusy(true);
    try {
      await apiDeleteNoContent(`procurement/supplier-returns/${supplierReturnId}/lines/${line.id}`);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      setBusy(false);
