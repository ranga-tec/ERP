...
export function RfqLineRow({
  ...
  async function saveEdit() {
    ...
    try {
      ...
      await apiPutNoContent(`procurement/rfqs/${rfqId}/lines/${line.id}`, {
        quantity: qty,
  ...
  async function deleteLine() {
    if (!window.confirm("Delete this RFQ line?")) return;

    setError(null);
    setBusy(true);
    try {
      await apiDeleteNoContent(`procurement/rfqs/${rfqId}/lines/${line.id}`);
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      setBusy(false);
